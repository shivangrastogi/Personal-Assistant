...
from DATA.TTS_MINIMAL.config.shared_configs import *
from DATA.TTS_MINIMAL.utils.generic_utils import find_module
...
#     # TODO: fix this
#     if model_name == "xtts":
#         from DATA.TTS_MINIMAL.tts.configs.xtts_config import XttsConfig
#
#         config_class = XttsConfig
#     paths = ["DATA.TTS_MINIMAL.tts.configs", "DATA.TTS_MINIMAL.vocoder.configs", "DATA.TTS_MINIMAL.encoder.configs", "DATA.TTS_MINIMAL.vc.configs"]
#     for path in paths:
#         try:
...
def register_config(model_name: str) -> Coqpit:
    ...
    if model_name == "xtts":
        from DATA.TTS_MINIMAL.tts.configs.xtts_config import XttsConfig
        config_class = XttsConfig
    elif model_name == "fast_pitch":
        from DATA.TTS_MINIMAL.tts.configs.fast_pitch_config import FastPitchConfig
        config_class = FastPitchConfig

    # fallback dynamic discovery
    if config_class is None:
        paths = [
            "DATA.TTS_MINIMAL.tts.configs",
            "DATA.TTS_MINIMAL.vocoder.configs",
            "DATA.TTS_MINIMAL.encoder.configs",
            "DATA.TTS_MINIMAL.vc.configs",
        ]
        for path in paths:
            try:
                config_class = find_module(path, config_name)
